...
function waterCreatTable() {
    ...
    db.selectSql({
        ...
        if (ret.status) {
            ...
            //创建停水通知单本地数据表--POSTING_LIST  2020-09-04--zlx
            var createSql = db.executeSqlSync({
                name: 'test',
                sql: 'CREATE TABLE POSTING_LIST('+
                     ...
                     'Remark TEXT,'+            // 备注
                     'TaskNo TEXT,'+            // 任务编号
                     'OperatedTime TEXT,'+      // 创建时间
                     'DispatchTime TEXT,'+      // 分派时间
                     'UseTime TEXT,'+           // 领用时间
                     'HandleTime TEXT,'+        // 办理时间
                     'AuditTime TEXT,'+         // 审核时间
                     'AuditTimes TEXT,'+        // 审核不通过次数
                     'Files TEXT,'+             // 当前任务所有的文件信息
                     'Handles TEXT,'+           // 当前任务所有的操作信息

                     'SaveTime TEXT,'+          // 保存时间
                     'SaveRemark TEXT,'+        // 保存备注
                     'SavePhoto TEXT,'+         // 保存照片
                     'PhotoLocation TEXT,'+     // 照片经纬度
                     'isSave TEXT,'+            // 是否保存   0-未保存  1-已保存
                     'isUpload TEXT,'+          // 是否上传   0-未上传  1-已上传
                     'isFail TEXT,'+            // 是否上传失败   0-未上传失败  1-已上传失败
                     'failRemark TEXT,'+        // 上传失败原因
                     'userName TEXT,'+          // 当前登录人员用户名
                     'ReserveOne TEXT,'+        // 预留字段
                     'ReserveTwo TEXT,'+        // 预留字段
                     'ReserveThree TEXT,'+      // 预留字段
                     'ReserveFour TEXT,'+       // 预留字段
                     'ReserveFive TEXT)'        // 预留字段
            });
...
function dataQuery(fromCb, callback) { //fromCb 表示不是抄表管家更新基础数据 ，true表示是更新抄表管家基础数据
    ...
    var ret = db.selectSqlSync({
        name: 'CBtest',
        sql: 'SELECT * FROM MRM_USER_BEAN where CBBZ=1 and ZTSCCG=0 and CWXX=" " and userName ="' + CurentUserName + '"'
    });
    if (ret.status) {
        if (ret.data.length > 0) {
