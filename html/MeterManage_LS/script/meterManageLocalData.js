...
function PostingList(WorkTypeId, userName, item) {
    ...
    db.selectSql({
        name: 'Wsdatabase',
        sql: "SELECT * FROM POSTING_LIST where Status = '7' AND isUpload = '0' and TaskNo = '" + WorkTypeId + "' AND userName = '" + userName + "'"
    }, function(ret, err){
        if ( ret.status ) {
                ...
                api.ajax({
                    ...
                    if (ret) {
                        ...
                        if (ret.Status == '0') {
                          ...
                          var addArr = [];
                          if (dataArr.length != dataBase.length) {
                              for (var i = 0; i < dataArr.length; i++) {
                                  for (var j = 0; j < dataBase.length; j++) {
                                    var float = true;
                                    if (dataArr[i].CustomerCode == dataBase[j].CustomerCode) {
                                        float = false;
                                        break;
                                    } else {
                                        float = true;
                              ...
                              for (var i in addArr) {
                                var insertSql = "INSERT INTO POSTING_LIST("+
                                                ...
                                                "'"+ addArr[i].Name +"',"+
                                                "'"+ addArr[i].Status +"',"+
                                                "'"+ addArr[i].CustomerCode +"',"+
                                                "'"+ addArr[i].CustomerName +"',"+
                                                "'"+ addArr[i].Address +"',"+
                                                "'"+ addArr[i].Location +"',"+
                                                "'"+ addArr[i].Nature +"',"+
                                                "'"+ addArr[i].Caliber +"',"+
                                                ...
                                                "'"+ addArr[i].EndScale +"',"+
                                                "'"+ addArr[i].Amount +"',"+
                                                "'"+ addArr[i].ArrearMoney +"',"+
                                                "'"+ addArr[i].Description +"',"+
                                                "'"+ addArr[i].Remark +"',"+
                                                "'"+ addArr[i].TaskNo +"',"+
                                                "'"+ addArr[i].OperatedTime +"',"+
                                                "'"+ addArr[i].DispatchTime +"',"+
                                                "'"+ addArr[i].UseTime +"',"+
                                                "'"+ addArr[i].HandleTime +"',"+
                                                "'"+ addArr[i].AuditTime +"',"+
                                                "'"+ addArr[i].AuditTimes +"',"+
...
// 首次认领数据保存至本地  zlx 2020-09-21
function firstDownloadData(WorkTypeId, userName, WorkTypeCode) {
            ...
            if (ret.Status == '0') {
              ...
              for (var i in dataArr) {
                var insertSql = "INSERT INTO POSTING_LIST("+
                                ...
                                "'"+ dataArr[i].Name +"',"+
                                "'"+ dataArr[i].Status +"',"+
                                "'"+ dataArr[i].CustomerCode +"',"+
                                "'"+ dataArr[i].CustomerName +"',"+
                                "'"+ dataArr[i].Address +"',"+
                                "'"+ dataArr[i].Location +"',"+
                                "'"+ dataArr[i].Nature +"',"+
                                "'"+ dataArr[i].Caliber +"',"+
                                ...
                                "'"+ dataArr[i].StampNo +"',"+
                                "'"+ dataArr[i].LastScale +"',"+
                                "'"+ dataArr[i].BeginScale +"',"+
                                "'"+ dataArr[i].EndScale +"',"+
                                "'"+ dataArr[i].Amount +"',"+
                                "'"+ dataArr[i].ArrearMoney +"',"+
                                "'"+ dataArr[i].Description +"',"+
                                "'"+ dataArr[i].Remark +"',"+
                                "'"+ dataArr[i].TaskNo +"',"+
                                "'"+ dataArr[i].OperatedTime +"',"+
                                "'"+ dataArr[i].DispatchTime +"',"+
                                "'"+ dataArr[i].UseTime +"',"+
                                "'"+ dataArr[i].HandleTime +"',"+
                                "'"+ dataArr[i].AuditTime +"',"+
                                "'"+ dataArr[i].AuditTimes +"',"+
                                "'"+ dataArr[i].Files +"',"+
                                "'"+ dataArr[i].Handles +"',"+
...
function savePostingList1(taskNoAll, taskNoArr) {
    api.ajax({
        url: $api.getStorage("bwapipath") + '/api/waterMeters/info',
        method: 'post',
        dataType: 'json',
...
function createReviewBasicData() {
    ...
    var basicSql = "CREATE TABLE Review_TASK_BASIC_LIST(" +
                   ...
                   "ReaderName TEXT," +          //抄表员名称
                   "ReaderPhone TEXT," +         //抄表员电话
                   "ApplyLon TEXT," +            //工单申请经纬度
                   "ApplyLat TEXT," +            //工单申请经纬度
                   "SaveTime TEXT," +            //保存时间
                   "userName TEXT," +            //当前登录用户名
                   "isUploadAndSave TEXT," +     //默认 0-待办  1-已上传 2-已保存
                   "SurveyRemark TEXT," +        //上一节点备注
                   "fieldOne TEXT," +            //预留字段
                   "fieldTwo TEXT," +            //预留字段
                   "fieldThree TEXT," +          //预留字段
                   "fieldFour TEXT," +           //预留字段
                   "fieldFive TEXT)";            //预留字段
   db.executeSql({
       name: 'Wsdatabase',
...
function createReviewHandledData() {
    ...
    var handledSql = "CREATE TABLE Review_TASK_HANDLED_LIST(" +
                     ...
                     "Remark TEXT," +                //处理结果
                     "oldTableEndVal TEXT," +        //旧表止度
                     "newTableEndVal TEXT," +        //新表起度
                     "oldTableNo TEXT," +            //原表表号
                     "newTableNo TEXT," +            //新表表号
                     "billingTime TEXT," +           //计费时间
                     "billingTimeId TEXT," +         //
                     "newTableCaliber TEXT," +       //新表口径
                     "newTableCaliberId TEXT," +     //
                     "newTableType TEXT," +          //新表类型
                     "newTableTypeId TEXT," +        //
                     "userAutograph TEXT," +         //用户签名
...
function bwcreateAuditData() {
    var db = api.require("db");
    var createSql = "CREATE TABLE AUDIT_TASK_LIST(" +
                    "Id TEXT," +                 //工单Id
                    "Source TEXT," +             //工单来源
                    "Code TEXT," +               //工单类型编码
                    "Name TEXT," +               //工单类型名称
