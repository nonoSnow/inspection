...
function createAuditData() {
  ...
  var createSql = "CREATE TABLE AUDIT_TASK_LIST(" +
                  ...
                  "fieldFour TEXT," +          //预留字段
                  "fieldFive TEXT)";           //预留字段
  var createResult = db.executeSqlSync({
    name: 'Wsdatabase',
    sql: Sql
  });
...
//插入稽核数据  2020-09-07 zlx
function insertAuditData(taskInfo) {
  var db = api.require("db");
...
//查询任务详情保存至本地
function saveAuditList(auditTaskNoAll) {
  var db = api.require("db");
  api.ajax({
      url: $api.getStorage("jhapipath") + 'waterMeters/info',
      method: 'post',
