...
function calibreRenderingthematicMap(data) {
    ...
    if (data.all) {
        // var url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGisLine&outputFormat=application%2Fjson`;
        var url = getAppMapSetConfig.diameterMapAll;
    } else {
        // var url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetUnitType&outputFormat=application%2Fjson&viewparams=min:${data.min};max:${data.max}`;
    ...
    LineselectSingleClick.on('select', function(e) {
        ...
        if (features.length > 0) {
            ...
            var renderData = {
                ...
                length: (property.length * 100000).toFixed(5), //管线管长
...
function pipeRenderingthematicMap(name) {
    ...
    if (name == true) {
        ...
        // var url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetLineByName&outputFormat=application%2Fjson&viewparams=name:${name}`;
...
function pipeNetworkRenderingthematicMap(data) {
    ...
    if (data.type == 'point') {
        if (data.all) {
            // url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGisPoint&outputFormat=application%2Fjson`;
           url = getAppMapSetConfig.pointMapAll;
        } else {
            // url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetPointStateClass&outputFormat=application%2Fjson&viewparams=StateClass:${data.value}`;
           url = getAppMapSetConfig.pointMapClass+`:${data.value}`;
        }
    } else {
        if (data.all) {
          // url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGisLine&outputFormat=application%2Fjson`;
          url = getAppMapSetConfig.lineMapAll;
        } else {
          // url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetLineStateClass&outputFormat=application%2Fjson&viewparams=StateClass:${data.value}`;
...
function pointRenderingthematicMap(name) {
  map.removeInteraction(LineselectSingleClick);
    ...
    removeAddLayer('thematiclayer', 18);
    var layerName = name;
    // var url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetPointByName&outputFormat=application%2Fjson&viewparams=name:${layerName}`;
      var url=getAppMapSetConfig.singleMap+`:${layerName}`;
    if (layerName == 'famenlei' || layerName == 'guanjianlei' || layerName == 'qitalei') {
        // url = `http://119.3.192.111:5431/geoserver/OpenGIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=OpenGIS%3AGetPointByNameType&outputFormat=application%2Fjson&viewparams=name:${layerName}`;
...
function clickViewGetDataLine(layerName, type, isAddPoint) {
    // isAddPoint  用于判断是显示信息还是添加图标
    point_pipe_clickObj.layerLineName = layerName;
    point_pipe_clickObj.LineType = type;
    point_pipe_clickObj.isAddPoint = isAddPoint;
...
var lineClickSearch = function(evt) {
    var view = map.getView();
    var viewResolution = view.getResolution();
    var source = point_pipe_clickObj.layerLineName.getSource();
    var url = source.getGetFeatureInfoUrl(evt.coordinate, viewResolution, view.getProjection(), {
        'INFO_FORMAT': 'application/json',
        'FEATURE_COUNT': 1
    });
                ...
                if (data.features.length != 0) {
                    var result = data.features[0].properties;
                        ...
                        var renderData = {
                            ...
                            location: result.Location, //所属道路
                            buildtime: result.BuildTime, //埋设日期
                            length: (result.length * 100000).toFixed(5),
