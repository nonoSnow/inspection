...
function layerActive(that, type) {
    ...
    if (type == '2D') {
        map.addLayer(getTdtLayer("vec_w"));
        map.addLayer(getTdtLayer("cva_w"));
        ...
        map.addLayer(getTdtLayer("img_w"));
        map.addLayer(getTdtLayer("cia_w"));
        $(that).siblings().children().find('.layerimg').removeClass('layer_moon_img_border');
        $(that).children().find('.layerimg').addClass('layer_moon_img_border');
        $('#moonchecked').show();
        $('#layerchecked').hide();
...
function GetAllCollectPoint(type, Type) { //type 为add或者remove , Type 为Point或者Line
    if (type == 'add') {
        fnGet(`services/OpenGIS/PipeCollect/GetAllCollect?type=${Type}`, null, false, function(ret, err) {
            if (ret && ret.success) {
...
function MapNavigation(navigator, type) {
    ...
    bMap.getLocation({
        ...
        if (ret.status) {
            ...
            //           console.log((ret.distance/1000)+'km');
            //         }else {
            //           console.log((ret.distance/1000)+'m');
            //         }
            //     }
            // });
...
function deletePoint(data) {
    fnGet(`services/OpenGIS/EditPipeManage/DeletePoint?Number=${data.pointnumbe}&Wkt=${data.wkt}&PointName=${data.pointtype}&PointElevation=${data.elevation}&PointDeep=${data.deep}&Location=${data.location}&BuildTime=${data.buildtime}&Firm=${data.firm}&BuildUnit=${data.buildunit}&UnitType=${data.unittype}&StateClass=${data.stateClass}`, null, true, function(ret, err) {
        if (ret && ret.success) {
            api.toast({
                msg: '已提交后台审核!',
                duration: 2000,
...
function GetAllLabel(type) {
    if (type == 'add') {
        fnGet('services/OpenGIS/LabelManage/GetAllLabel', null, false, function(ret, err) {
            if (ret && ret.success) {
                var result = ret.result;
