...
function bookmarkPointOrPipeLine(that, type,from = null) {
    ...
    if ($(that).hasClass('no_bookmark')) {
        ...
        fnGet(`services/OpenGIS/PipeCollect/DeleteCollect?number=${params.number}`, null, true, function(ret, err) {
            if (ret && ret.success) {
               api.toast({
                   msg: '移除收藏夹成功!',
                   duration: 2000,
...
function IsCollect(id, type) {
    //  获取数据添加为收藏
    fnPost(`services/OpenGIS/PipeCollect/IsCollect?number=${id}`, null, 'application/json', true, false, function(ret, err) {
        if (ret && ret.success) {
            if (ret.result) {
                //已经收藏
                if (type == 'point') {
...
function searchAllClass(type,element,defaultValue = null){
  var options = options;
  fnPost(`services/OpenGIS/SearchPipe/SearchAllClass?fieldname=${type}`,null,'application/json',true,false,function(ret,err){
    if(ret && ret.success){
...
function searchStateClass(element,defaultValue = null){
  fnPost(`services/OpenGIS/SearchPipe/SearchStateClass`,null,'application/json',true,false,function(ret,err){
    api.hideProgress();
    if(ret && ret.success){
      var result = ret.result;
      var options = '<option value="">请选择</option>';
...
function createNumber(type,callback){
     fnPost(`services/OpenGIS/EditPipeManage/CreatNumber?type=${type}`,null,'application/json',true,false,function(ret,err){
       if(ret && ret.success){
        callback(ret.result);
      } else {
        api.toast({
...
function addNewPoint(body,callback){
  fnPost(`services/OpenGIS/EditPipeManage/AddPoint`,body,'application/json',true,false,function(ret,err){
    api.hideProgress();
    if(ret && ret.success){
      callback(ret.result);
    } else {
...
function searchByNumber(data,type){
  ...
  fnPost('services/OpenGIS/SearchPipe/SearchByNumber?input='+number,null,'application/json',true,false,function(ret,err){
    if(ret && ret.success){
      if(type == 'Point'){
...
function IsCheckState(number,callback){
  fnPost(`services/OpenGIS/EditPipeManage/IsCheckState?input=${number}`, {}, 'application/json', true, false, function(ret, err) {
      if (ret && ret.success) {
        if(ret.result){
          callback(true);
        }else {
